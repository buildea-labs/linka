...
|---|---|
| Android (`linkaAndroidKotlin/`) | `MaterialTheme` com `ColorScheme` customizado, componentes MD3 nativos do Jetpack Compose (`FilledButton`, `NavigationBar`, `ModalBottomSheet`, etc.) |
| PWA (`linkaSpeedtestPwa/`) | CSS Custom Properties como tokens — convencao MD3 adaptada para web |

> Nota: o projeto Android usa **Kotlin + Jetpack Compose** (nao Flutter/Dart). O tema e definido em
> `linkaAndroidKotlin/linka-android-kotlin/app/src/main/kotlin/io/linka/app/kotlin/ui/LinkaTheme.kt`.

---

## 2. Paleta de cores — Android (Kotlin/Compose)

Arquivo de referencia: `linkaAndroidKotlin/linka-android-kotlin/app/src/main/kotlin/io/linka/app/kotlin/ui/LinkaTheme.kt`
...
### Android — escala tipografica (Kotlin/Compose)

Fonte principal: **Geist** (para display e body). Definida em `LinkaTheme.kt` via `FontFamily` do Compose.

| Papel | Tamanho | Peso | Tracking | Line-height |
|---|---|---|---|---|
| `h1` (displayLarge) | 34sp | 700 | -0.015em | 36/34 |
| `h2` (displayMedium) | 20sp | 600 | -0.005em | 24/20 |
| `h3` (displaySmall) | 15sp | 500 | neutro | 20/15 |
| `body` | 14sp | 400 | neutro | 20/14 |
| `secondaryMetric` | 17sp | 600 | -0.005em | 20/17 |
| `label` | 12sp | 400 | +0.01em | 16/12 |

Valores numericos de metricas usam fonte monoespaced com `fontFeatureSettings = "tnum"` (tabular figures).
...
|---|---|
| Android (Kotlin/Compose) | `linkaAndroidKotlin/linka-android-kotlin/app/src/main/kotlin/io/linka/app/kotlin/ui/LinkaTheme.kt` |
| PWA | tokens.css (dentro de `linkaSpeedtestPwa/src/`) |
