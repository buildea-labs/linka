...
O histórico de medições, análises (`history.ts`, `historyTrends.ts`, `historyInsights.ts`) e preferências de usuário são armazenados diretamente no `localStorage` do navegador, utilizando as abstrações de utils sem dependência de bancos de dados nativos.
