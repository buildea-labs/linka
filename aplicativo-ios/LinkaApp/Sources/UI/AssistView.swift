...
// Este arquivo ultrapassa 400 linhas pois concentra toda a lógica de view, navegação e widgets do fluxo do Assist.
// Separar os subcomponentes sem necessidade reutilizável apenas espalharia a lógica de uma feature muito isolada.
import SwiftUI
...
struct ChatMessage: Identifiable {
    let id = UUID()
    var payload: BubblePayload
    let isUser: Bool

    init(payload: BubblePayload, isUser: Bool) {
        self.payload = payload
        self.isUser = isUser
    }
    
    init(text: String, longText: String? = nil, isUser: Bool) {
        self.payload = .text(text, longText: longText)
        self.isUser = isUser
    }
    
    var text: String {
        get { if case .text(let s, _) = payload { return s } else { return "" } }
        set { if case .text(_, let l) = payload { payload = .text(newValue, longText: l) } else { payload = .text(newValue, longText: nil) } }
    }
    
    var longText: String? {
        get { if case .text(_, let l) = payload { return l } else { return nil } }
        set { if case .text(let s, _) = payload { payload = .text(s, longText: newValue) } }
    }
}
    ...
    private func bubble(for msg: ChatMessage) -> some View {
        HStack {
            if msg.isUser {
                ...
                .padding(.vertical, 8)
                // Removido o background colorido e padding horizontal da IA
