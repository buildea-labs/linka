...
public struct LocalGatewayDiscovery: LocalGatewayDiscovering {
    ...
    public func discoverPrimaryInterface() -> LocalInterfaceInfo? {
        ...
        while let current = ptr {
            ...
            if isUp && isRunning && !isLoopback && name.hasPrefix("en"),
               ...
               addr.pointee.sa_family == UInt8(AF_INET) {
                ...
                let gateway = Self.resolveGateway(forInterface: name)
                ...
                let info = LocalInterfaceInfo(name: name, ipv4Address: ipStr, netmask: netmaskStr, gatewayCandidate: validGateway)
    ...
    /// Consulta a tabela de roteamento via sysctl para descobrir o gateway da interface
    public static func resolveGateway(forInterface name: String) -> String? {
        #if canImport(Darwin)
