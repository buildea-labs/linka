...
## 2. Módulos Android (16 módulos)

### Módulos Core (5)

| Módulo | Responsabilidade | Arquivos-chave |
|---|---|---|
| `:app` | Entry point, telas transversais, navegação, tema | `LinkaApplication.kt`, `MainActivity.kt`, `AppNavGraph.kt`, `AppShell.kt`, `LinkaTheme.kt` |
| `:coreDatabase` | Persistência Room — DAOs, entidades, migrations | `LinkaDatabase.kt`, `MedicaoDao.kt`, `MedicaoEntity.kt`, `ApelidoDispositivoDao.kt`, `ApelidoDispositivoEntity.kt` |
| `:coreDatastore` | Preferências chave-valor via Jetpack DataStore | `PreferenciasAppRepository.kt`, `CoreDatastoreModulo.kt` — DataStore name: `"linkaPreferencias"` — 14 chaves: monitoramentoAtivo, modemHost/Username/Password/PermanecerConectado, temaSelecionado, analiseAvancada, nomeUsuario, fotoUriUsuario, operadora, planoInternet, regiao, limiteAlertaMbps, ultimaVerificacaoMonitoramento + 4 chaves de histerese de alerta (latencia, dns, rssi, semInternet) |
| `:coreNetwork` | Monitoramento de rede, estado de conexão | `MonitorRede.kt`, `MonitorRedeAndroid.kt`, `EstadoConexao.kt`, `SnapshotRede.kt`, `WifiLinkSnapshot.kt`, `CoreNetworkModulo.kt` |
| `:corePermissions` | Gerenciamento de permissões runtime | [não documentado em código lido — verificar em `corePermissions/src/`] |
| `:coreTelephony` | Acesso a SIM e dados celulares | [não documentado em código lido — verificar em `coreTelephony/src/`] |

### Módulos de Feature (10+)

| Módulo | Responsabilidade | Tela(s) | Arquivos-chave |
|---|---|---|---|
| `:featureHome` | Visão geral status conexão, quicklinks | `HomeScreen.kt` | [não documentado em detalhe — verificar em `featureHome/src/`] |
| `:featureSpeedtest` | Motor de speedtest, fases, resultado | `SpeedTestScreen.kt` (tela em `:app`), `ResultadoVelocidadeScreen.kt` (tela em `:app`) | `ExecutorSpeedtestCloudflare.kt`, `ModoSpeedtest.kt`, `ResultadoSpeedtest.kt`, `SpeedtestQualityClassifier.kt`, `SnapshotExecucaoSpeedtest.kt`, `EstadoExecucaoSpeedtest.kt`, `FaseSpeedtest.kt`, `FeatureSpeedtestModulo.kt` |
| `:featureWifi` | Scan Wi-Fi, análise de redes vizinhas | `WifiScreen.kt` | `ResumoWifi.kt`, `MontarResumoWifiUseCase.kt`, `RedeVizinha.kt`, `ScannerRedesWifi.kt`, `SnapshotScanWifi.kt`, `FeatureWifiModulo.kt` |
| `:featureDevices` | Lista de dispositivos na rede local | `DispositivosScreen.kt` (tela em `:app`) | [não documentado em detalhe — verificar em `featureDevices/src/`] |
| `:featureDns` | Benchmark de servidores DNS | `DnsScreen.kt` (tela em `:app` ou módulo) | `BenchmarkDnsDoh.kt`, `BenchmarkDns.kt`, `ResultadoBenchmarkDns.kt`, `SnapshotBenchmarkDns.kt`, `EstadoBenchmarkDns.kt`, `AvaliadorCoerenciaDns.kt`, `OrientadorConfiguracaoDns.kt`, `FeatureDnsModulo.kt` |
| `:featureDiagnostico` | Diagnóstico guiado, IA assistente (Orbit) | `DiagnosticoScreen.kt`, `OrbitScreen.kt` (telas em `:app`) | `DiagnosticStatus.kt`, `SnapshotDiagnostico.kt` — [ViewModels e serviços não documentados em detalhe] |
| `:featureFibra` | Diagnóstico de fibra ótica via modem Nokia GPON | `FibraScreen.kt` (tela em `:app`) | `ExecutorFibra.kt`, `NokiaModemClient.kt`, `NokiaModemParser.kt`, `NokiaModemCrypto.kt`, `ClassificadorSaudeGpon.kt`, `SnapshotFibra.kt`, `GponStatus.kt`, `WanStatus.kt`, `PppStatus.kt`, `DeviceInfoFibra.kt`, `FeatureFibraModulo.kt` |
| `:featureHistory` | Histórico de testes | `HistoricoScreen.kt` (tela em `:app`) | `MedicaoDao.kt` (queries no `:coreDatabase`), `ExportHistoricoBottomSheet.kt` |
| `:featureSettings` | Preferências do usuário | `AjustesScreen.kt` (tela em `:app`) | `PreferenciasAppRepository.kt` (em `:coreDatastore`) — o módulo `FeatureSettingsModulo.kt` é vazio (stub) |

> **Nota:** Documentação existente nos módulos de feature é majoritariamente inferencial. Os únicos módulos com documentação validada no código são `:coreNetwork` (via `MonitorRede.kt`) e `:featureWifi` (documentado em `WIFI_FEATURES.md`).
